...
public class MovementController2D : MonoBehaviour
{
    ...
    private SingleMonsterController _mon;
    ...
    void Start()
    {
        ...
        _mon = GetComponent<SingleMonsterController>();
    }
    ...
    void Update()
    {
        ...
        if (pathLeftToGo.Count > 0) //if the target is not yet reached
        {
            ...
            transform.position += dir.normalized * _mon._stat.speed * Time.deltaTime;
            if (((Vector2)transform.position - pathLeftToGo[0]).sqrMagnitude < _mon._stat.speed * Time.deltaTime * _mon._stat.speed * Time.deltaTime) 
            {
                transform.position = pathLeftToGo[0];
                pathLeftToGo.RemoveAt(0);
            }
        }
        else if(_mon._canChase && Vector2.Distance(_mon.Target.transform.position, transform.position) > _mon._stat.attackRange)
        {
            transform.position = Vector3.MoveTowards(transform.position, _mon.Target.transform.position, _mon._stat.speed * Time.deltaTime);
        }
