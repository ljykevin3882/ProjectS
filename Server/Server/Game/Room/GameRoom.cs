...
namespace Server.Game
{
    public class GameRoom : JobSerializer
    {
        ...
        public void Update()
        {
            ...
            //{
            //    //Console.WriteLine($"플레이어 ID: {p.Id}: 골드: {p.Gold}");
            //    Console.WriteLine($"플레이어 위치: ({p.PosInfo.PosX}, {p.PosInfo.PosY})");
            //}
        }
        public void EnterGame(GameObject gameObject)
        {
            ...
            if (type == GameObjectType.Player)
            {
                ...
                //{
                //    player.Info.PosInfo.PosX = -73;
                //    player.Info.PosInfo.PosY = 80;
                //    player.Info.PosInfo.RotZ = 0;
                //}
                ...
                //else if (_players.Count == 5)
                //{
                //    player.Info.PosInfo.PosX = -69.1f;
                //    player.Info.PosInfo.PosY = 50.9f;
                //    player.Info.PosInfo.RotZ = 0;
                //}
                //else if (_players.Count == 6)
                //{
                //    player.Info.PosInfo.PosX = 69;
                //    player.Info.PosInfo.PosY = 51;
                //    player.Info.PosInfo.RotZ = 0;
                //}
                //else if (_players.Count == 7)
                //{
                //    player.Info.PosInfo.PosX = 64;
                //    player.Info.PosInfo.PosY = -52;
                //    player.Info.PosInfo.RotZ = 0;
                //}
                //else if (_players.Count == 8)
                //{
                //    player.Info.PosInfo.PosX = -86;
                //    player.Info.PosInfo.PosY = -54;
                //    player.Info.PosInfo.RotZ = 0;
                //}
        ...
        public void HandleSkill(Player player, C_Skill skillPacket)
        {
            ...
            // TODO - 스킬 사용 조건 아래와 같이 넣어주기
            //if (info.PosInfo.State != CreatureState.Idle)
            // 통과 되면 state skill로 바꿔주기
            ...
            //skill.PlayerRotz = skillPacket.PlayerRotz;
            // TODO - 우선 스킬아이디 테스트용으로 1 설정
            // DATA SHEET 빼서 JSON으로 관리
            // 1. 평타
            Broadcast(skill);
            // 플레이어 회전값 서버에 반영
            //player.PosInfo.RotZ = skillPacket.PlayerRotz;
            // 샷건 총알 각도
        ...
        // TODO - 리팩토링 -> 플레이어가 총을 Creature에게 맞췄을 때 로직
        public void HandleHitPlayer(Player hitter, GameObject enemy, GameObjectType enemyType, C_HitPlayer hitPacket)
        {
            ...
            if (bullet.IsTouch == false)
            {
                ...
                Console.WriteLine($"hitter {hitter.Id} Attack: {hitter.Stat.Attack}");
                Console.WriteLine($"enemy {enemy.Id} HP: {enemy.Stat.Hp}");

            }
            else
            {
                //Console.WriteLine("HandleHitPlayer Bullet already touched!");
            }

        }

        // TODO - 리팩토링 -> 플레이어가 몬스터를 때렸을 때
        public void HandleHitMonster(Player hitter, GameObject enemy, GameObjectType enemyType, C_HitMonster hitPacket)
        {
            ...
            {
                //Console.WriteLine("HandleHitMonster Bullet already touched!");
            }
        ...
        // TODO - 리팩토링 - Hit 시리즈는 전부 하나의 함수에서 하게 하기
        // 또는 서버에서 전부 처리하게 하기
        // 몬스터가 플레이러를 때렸을 때
        ...
        public void HandleChangeRotZ(Player player, C_ChangeRotz rotZPacket)
        {
            ...
            //Console.WriteLine("클라에서 총알 각도: "+player.PosInfo.RotZ);
            //Console.WriteLine("플레이어 각도: " + rotZ.RotZ);
            //Console.WriteLine("플레이어 Id: " + rotZ.ObjectId);
            Broadcast(rotZ);
        }
        ...
        void HandleCloseAreaText()
        {
            ...
            if (_closedAreaIds.Count != 4)
            {
                ...
                //Console.WriteLine($"2. areaId: {areaId}");
                //Console.WriteLine($"2. _closedAreaIds: { _closedAreaIds[_closedAreaIds.Count - 1]}");
                //Console.WriteLine($"2. Map: {Map.Areas[areaId].Type}");
        ...
        void HandleEndGame()
        {
            Console.WriteLine($"End Game Room {RoomId}");
            Random random = new Random();
            int winnerIndex = random.Next(0, _players.Count);
            int count = 0;
            Console.WriteLine($"Remain player: {_players.Count} players");
            foreach (Player player in _players.Values)
            {
                if (winnerIndex != count)
                {
                    count++;
                    Console.WriteLine("Player " + player.Id + " Dead");
                    LeaveGame(player.Id);
                }
            }
            lastTimer.Stop();
        ...
        {
            //Console.WriteLine($"3. areaId: {area.Id}");
            //Console.WriteLine($"3. _closedAreaIds: {_closedAreaIds[_closedAreaIds.Count - 1]}");
            //Console.WriteLine($"3. Map: {area.Type}");
            foreach (Player player in _players.Values)
            {
                ...
                if (lastClosedArea.IsInArea(player.PosInfo))
                {
                    ...
                    Console.WriteLine($"Player {player.Id} is in Cloesd Area so Dead");
                }
            }
        }
        ...
        int GetRandomClosingAreaId()
        {
            ...
            //Console.WriteLine($"1. areaId: {randIndex}");
            //Console.WriteLine($"1. Map: {Map.Areas[randIndex].Type}");
            return randIndex;
        }
